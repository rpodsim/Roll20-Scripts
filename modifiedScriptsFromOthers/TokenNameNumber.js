...
var TokenNameNumber = TokenNameNumber || (function () {
	...
	var version = '0.5.3',
				...
				case 0.2:
					state.TokenNameNumber = _.defaults({
						...
						config: {
							randomSpace: 5,
							useDots: false,
							dots: ['brown', 'yellow', 'green', 'blue', 'purple']
						}
		...
		getConfigOption_RandomSpace = function () {
			var text = (state.TokenNameNumber.config.randomSpace > 0 ? '<span style="color: green; font-weight:bold;">' + state.TokenNameNumber.config.randomSpace + '</span>' : '<span style="color: red; font-weight:bold;">Off</span>');
			return '<div>' +
		...
		getConfigOption_UseDots = function () {
			var text = (state.TokenNameNumber.config.useDots ? '<span style="color: green; font-weight:bold;">On</span>' : '<span style="color: red; font-weight:bold;">Off</span>');
		...
		handleInput = function (msg) {
			...
			case '!tnn-config':
				...
				if (!args.length) {
					sendChat('', '/w ' + who + ' ' + '<div style="border: 1px solid black; background-color: white; padding: 3px 3px;">' + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' + 'TokenNameNumber v' + version + '</div>' + getAllConfigOptions() + '</div>');
					return;
				}
				_.each(args, function (a) {
					...
					case 'toggle-use-dots':
						...
						sendChat('', '/w ' + who + ' ' + '<div style="border: 1px solid black; background-color: white; padding: 3px 3px;">' + getConfigOption_UseDots() + '</div>');
						break;
					...
					case 'toggle-dot':
						...
						sendChat('', '/w ' + who + ' ' + '<div style="border: 1px solid black; background-color: white; padding: 3px 3px;">' + getConfigOption_Dots() + '</div>');
						break;


					case 'random-space':
						state.TokenNameNumber.config.randomSpace = parseInt(opt[0], 10);
						sendChat('', '/w ' + who + ' ' + '<div style="border: 1px solid black; background-color: white; padding: 3px 3px;">' + getConfigOption_RandomSpace() + '</div>');
						break;
		...
		getMatchers = function (pageid, represents) {
			var matchers = [];
			if (_.has(state.TokenNameNumber.registry, pageid) && _.has(state.TokenNameNumber.registry[pageid], represents)) {
		...
		setNumberOnToken = function (obj) {
			...
			if (_.contains(addTokenCache, obj.id) && 'graphic' === obj.get('type') && 'token' === obj.get('subtype')) {
				...
				if (tokenName.match(/%%NUMBERED%%/) || _.some(matchers, function (m) {
						return m.test(tokenName);
					})) {
					if (0 === matchers.length || !_.some(matchers, function (m) {
							return m.test(tokenName);
						})) {

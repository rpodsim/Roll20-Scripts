	...
	var version = 0.23,
		...
		HandleInput = function (msg) {
			...
			case '!mark':
				...
				if (errors.length) {
					sendChat('', '/w ' + who + '<div style="border: 1px solid black; background-color: white; padding: 3px 3px;">' + '<div><span style="font-weight:bold;color:#990000;">Error:</span> ' + errors.join('</div><div><span style="font-weight:bold;color:#990000;">Error:</span> ') + '</div>' + '</div>');
				}
...
on("ready", function () {
	...
	if ("undefined" !== typeof playerIsGM && _.isFunction(playerIsGM)) {
		Mark.CheckInstall();
		Mark.RegisterEventHandlers();
	} else {
		log('--------------------------------------------------------------');
		log('Mark requires the playerIsGM module to work.');
		log('playerIsGM GIST: https://gist.github.com/shdwjk/8d5bb062abab18463625');
		log('--------------------------------------------------------------');
	}
});